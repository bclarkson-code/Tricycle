...
def radd(tensor: Tensor, subscript: Subscript):
    """
    ...
    indicator = to_tensor(
        np.ones(indicator_shape, dtype=np.bool_),
        requires_grad=False,
        is_vector=tensor.is_vector,
    )

    new_subscript = Subscript.from_split([idx, indicator_indices], subscript.output)
